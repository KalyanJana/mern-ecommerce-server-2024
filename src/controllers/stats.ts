...
export const getDashboardStats = TryCatch(async(req, res, next)=>{
    ...
    if(myCache.has(key)){
        ...
        const lastMonth = {
            ...
            end: new Date(today.getFullYear(),today.getMonth() , 0), //let say today is 23rd Nov then Nov 0 means 31st Oct.
        }
        ...
        const lastMonthRevenue = thisMonthOrders.reduce((total, order)=> total + (order.total ||0), 0)
...
export const getPieCharts = TryCatch(async(req, res, next)=>{
    ...
    if(myCache.has(key)) {
        ...
        const netMergin = grossIncome - discount - productionCost - burnt - marketingCost
